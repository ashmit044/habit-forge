...
export const SpaceColonyScene: React.FC<SpaceColonySceneProps> = ({ growthStage }) => {
  return (
    <div className="relative w-full h-[320px] md:h-[380px] rounded-2xl overflow-hidden bg-gradient-to-b from-[#0b091a] via-[#110f2e] to-[#080718] border border-violet-500/20 shadow-2xl flex items-center justify-center select-none">
      ...
      >
        ...
        {growthStage >= 1 && (
          <g transform="translate(130, 240)" className="animate-grow">
            {/* Solar Mast */}
        ...
        {growthStage >= 2 && (
          <g transform="translate(600, 260)" className="animate-grow">
            {/* Base Ring */}
        ...
        {/* STAGE 3: Orbital Explorer Starship */}
        {growthStage >= 3 && (
          <g transform="translate(260, 150)" className="animate-float">
            {/* Shuttle Hull */}
            ...
            {/* Warp Thruster Exhaust */}
            <ellipse cx="0" cy="20" rx="8" ry="4" fill="#c084fc" filter="url(#purpleGlow)" />
          </g>
        )}

        {/* STAGE 4: Singularity Quantum Core */}
        {growthStage >= 4 && (
          <g transform="translate(540, 150)" className="animate-pulse-subtle">
        ...
        <g transform="translate(400, 220)" className="animate-sway">
          {/* Warp Ring (Stage 5) */}
          {growthStage >= 5 && (
      ...
      <div className="absolute top-4 left-4 z-20 flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-slate-900/80 backdrop-blur-md border border-violet-500/30 text-violet-400 text-xs font-semibold shadow-lg">
        <span className="w-2 h-2 rounded-full bg-violet-400 animate-ping" />
        Astra Zenith &bull; Stage {growthStage}/5
      </div>
    </div>
  );
};
